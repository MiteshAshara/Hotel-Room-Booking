	...
	if($st=="Conform")
	{
		...
		{	
			...
			if($troom=="Superior Room")
			{
															//$type_of_room = 320;
															//$type_of_room = $troom;
				$sql= "SELECT * FROM TYPE WHERE TYPE = 'superior room'";
				
				$sql="select * from type WHERE type='Superior Room' and price ";
				$result = mysqli_query($sql);
				$count = mysqli_num_rows($result);
				$row = mysqli_fetch_assoc($result);
				
				$type_of_room = $row['price'];
			}
			else if($troom=="Deluxe Room")
			{
															//$type_of_room = 220;

				$sql= "SELECT * FROM TYPE WHERE TYPE = 'deluxe room'";
				
				$sql="select * from type WHERE type='deluxe Room' and price ";
				$result = mysqli_query($sql);
				$count = mysqli_num_rows($result);
				$row = mysqli_fetch_assoc($result);
			...
			else if($troom=="Guest House")
			{
				...
				$sql= "SELECT * FROM TYPE WHERE TYPE = 'guest house'";
				
				$sql="select * from type WHERE type='guest house' and price ";
				$result = mysqli_query($sql);
				$count = mysqli_num_rows($result);
				$row = mysqli_fetch_assoc($result);
			...
			else if($troom=="Single Room")
			{
				...
				$sql= "SELECT * FROM TYPE WHERE TYPE = 'single room'";
				
				$sql="select * from type WHERE type='single Room' and price ";
				$result = mysqli_query($sql);
				$count = mysqli_num_rows($result);
				$row = mysqli_fetch_assoc($result);
				$type_of_room = $row['price'];
